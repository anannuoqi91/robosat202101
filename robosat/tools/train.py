...
#         sys.exit("Error: Unknown [opt][loss] value !")
#
#     train_loader, val_loader = get_dataset_loaders(model, dataset, args_workers)
#
#     num_epochs = model["opt"]["epochs"]
#     if resume >= num_epochs:
#         sys.exit("Error: Epoch {} set in {} already reached by the checkpoint provided".format(num_epochs, args_model))
#
#     history = collections.defaultdict(list)
#     log = Log(os.path.join(model["common"]["checkpoint"], "log"))
#
